...
if __name__ == "__main__":
    ...
    for row in tqdm(instances):
        if len(row["actual_documents"])>0:
            if len(row["actual_documents"]) >= 30:
    ...
    stats = []
    chances = 0
    for claim in tqdm(instances):

        matching = ds.claims.find({"file":claim["filename"], "idx":claim["read_idx"]})
        claim_ids = set()
        for match in matching:
            claim_ids.add(str(match["_id"]))

        done = 0

        for id in claim_ids:
            annotations = ds.annotations.find({"claim":ObjectId(id)})
        ...
        stats.append((len(claim_ids),done))

    print(stats)
    grand_total = 0
    grand_total2 = 0
    for total,done in stats:
        grand_total += total
        grand_total2 += done
