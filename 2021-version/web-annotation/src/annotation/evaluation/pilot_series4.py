...
for idx, claim in enumerate(ds.claims.find({"series":"boolq-sample-100claims-pilot-series3-variant-1"})):
    print("Claim")
    ...
    for anidx, annotation in enumerate(annotations):
        ...
        try:
            lines = get_wiki_clean_db(db, claim["page"].replace("_"," ").replace("-LRB-","(").replace("-RRB-",")"), redirects)["text"].split("\n")
        except:
            continue
        ...
        with open("pilot/series4/{}-{}.txt".format(claim['_id'],anidx),"w+") as f:
            f.write(out_text)

    with open("pilot/series4/timings.txt","w+") as f:
